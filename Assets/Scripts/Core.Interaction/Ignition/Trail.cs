...
public class Trail : MonoBehaviour
{
    ...
    public void OnCloserNodeChanged(Node current)
    {
        ...
        if (Emit)
        {
            if (current.handler == null || !current.handler.HasCompomponentOfType(OperationType.Ignite))
            {
                ...
                {
                    if (connection.handler != null && connection.handler.HasCompomponentOfType(OperationType.Ignite))
                    {
